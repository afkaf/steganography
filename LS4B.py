...
def edit_image(img, bits, nbits):
    ...
    shape = img.shape
    img = img.flatten()
    for i in tqdm(range(0,nbits//4+2)):
        try:
            threebits = bits[i*4:i*4+4].ljust(4,'0')
            threebitsv = int(threebits,2)%16
        except:
            break
        dif = img[i]%16-threebitsv
        old = img[i].copy()
        new = img[i].copy()
        new = (new + ((16-dif) if dif > 0 else -(16-abs(dif)))) if abs(dif) > 8 else new - dif
        new = new if new <= 255 else new - 16
        new = new if new >= 0 else new + 16
        img[i] = new
        # if i > (nbits//3+1)-50:
...
def read_image(img):
    ...
    img = img.flatten()
    for i in range(8):
        bitstr += bin(img[i] % 16)[2:].rjust(4,'0')
    size = int(bitstr,2)
    nbits = size//4+9
    bitstr = ''
    for i in tqdm(range(8,nbits)):
        try:
            bitstr += bin(img[i] % 16)[2:].rjust(4,'0')
        except: 
            break
...
if write:
    ...
    if im.shape[0]*im.shape[1]*im.shape[2]*4 > nbits+32:
        print('working...')
